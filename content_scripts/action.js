...
action.saveParameters = async () => {
	...
	if (
		!strategyData ||
		!strategyData.hasOwnProperty("name") ||
		!strategyData.hasOwnProperty("properties") ||
		!strategyData.properties
	) {
		await ui.showErrorPopup(
			"The current indicator/strategy do not contain inputs that can be saved."
		);
	...
	Object.keys(strategyData.properties).forEach((key) => {
		strategyParamsCSV += `${JSON.stringify(key)},${
			typeof strategyData.properties[key][0] === "string"
				? JSON.stringify(strategyData.properties[key])
				: strategyData.properties[key]
		}\n`;
	});
...
action.uploadSignals = async () => {
	await file.upload(
		signal.parseTSSignalsAndGetMsg,
		`Please check if the ticker and timeframe are set like in the downloaded data and click on the parameters of the "iondvSignals" script to automatically enter new data on the chart.`,
		true
	);
};
...
action.previewStrategyTestResults = async () => {
	...
	if (eventData.hasOwnProperty("message")) await ui.showPopup(eventData.message);
...
action.downloadStrategyTestResults = async () => {
	...
	if (bestResult && bestResult.hasOwnProperty(testResults.optParamName))
		await ui.showPopup(
			`The best found parameters are set for the strategy\n\nThe best ${
				testResults.isMaximizing ? "(max) " : "(min)"
			} ${testResults.optParamName}: ` + bestResult[testResults.optParamName]
		);
	file.saveAs(
		CSVResults,
		`${testResults.ticker}:${testResults.timeFrame} ${testResults.shortName} - ${
			testResults.cycles
		}_${testResults.isMaximizing ? "max" : "min"}_${testResults.optParamName}_${
			testResults.method
		}.csv`
	);
};
...
action.testStrategy = async (request, isDeepTest = false) => {
	try {
		...
		if (allRangeParams !== null) {
			// click cancel on parameters
			const testParams = await action._getTestParams(
				request,
				strategyData,
				allRangeParams,
				paramRange,
			...
			action._showStartMsg(
				testParams.paramSpace,
				testParams.cycles,
				testParams.backtestDelay
					? ` with delay between tests ${testParams.backtestDelay} sec`
					: ""
			);
			...
			if (testParams.shouldTestTF) {
				if (!testParams.listOfTF || testParams.listOfTF.length === 0) {
					await ui.showWarningPopup(
						`You set to test timeframes in options, but timeframes list after correction values is empty: ${testParams.listOfTFSource}\nPlease set correct one with separation by comma. \nFor example: 1m,4h`
					);
				} else {
					let bestValue = null;
					let bestTf = null;
					...
					for (const tf of testParams.listOfTF) {
						...
						if (testParams.hasOwnProperty("bestPropVal")) delete testParams.bestPropVal;
						if (testParams.hasOwnProperty("bestValue")) delete testParams.bestValue;
						testResults = await backtest.testStrategy(
							testParams,
							strategyData,
							allRangeParams
						); // TODO think about not save, but store them from  testResults.perfomanceSummary, testResults.filteredSummary = [], testResults.timeFrame to list
						...
						if (bestTf === null) {
							...
							testResults.isMaximizing
								? testParams.bestValue > bestValue
								: testParams.bestValue < bestValue
						) {
							bestValue = testResults.bestValue;
							bestTf = tf;
					...
					if (bestValue !== null) {
						await ui.showPopup(
							`The best value ${bestValue} for timeframe ${bestTf}. Check the saved files to get the best result parameters`
						);
					} else {
...
action._getRangeParams = async (strategyData) => {
	...
	if (changedStrategyParams.paramRange === null) {
		...
		typeof changedStrategyParams.paramRange === "object" &&
		Object.keys(changedStrategyParams.paramRange).length
	) {
		...
		throw new Error(
			"The strategy parameters invalid. Change them or run default parameters set."
		);
	}
...
action._getStrategyData = async (isDeepTest) => {
	...
	if (
		...
		throw new Error(
			"The current strategy do not contain inputs, than can be optimized. You can choose another strategy to optimize."
		);
	}
	return strategyData;
};
...
action._parseTF = (listOfTF) => {
	if (!listOfTF || typeof listOfTF !== "string") return [];
	return listOfTF
		.split(",")
		.map((tf) => tf.trim())
		.filter((tf) =>
			/(^\d{1,2}m$)|(^\d{1,2}h$)|(^\d{1,2}D$)|(^\d{1,2}W$)|(^\d{1,2}M$)/.test(tf)
		);
};

action._getTestParams = async (
	request,
	strategyData,
	allRangeParams,
	paramRange,
	...
	const testMethod =
		options.hasOwnProperty("optMethod") && typeof options.optMethod === "string"
			? options.optMethod.toLowerCase()
			: "random";
	let paramSpaceNumber = 0;
	if (["sequential"].includes(testMethod)) {
		paramSpaceNumber = Object.keys(allRangeParams).reduce(
			(sum, param) => (sum += allRangeParams[param].length),
			0
		);
	} else {
		paramSpaceNumber = Object.keys(allRangeParams).reduce(
			(mult, param) => (mult *= allRangeParams[param].length),
			1
		);
	}
	...
	if (
		!testParams.hasOwnProperty("startParams") ||
		!testParams.startParams.hasOwnProperty("current") ||
		!testParams.startParams.current
	) {
		throw new Error(
	...
	if (request.options) {
		testParams.isMaximizing = request.options.hasOwnProperty("isMaximizing")
			? request.options.isMaximizing
			: true;
		testParams.optParamName = request.options.optParamName
			? request.options.optParamName
			: backtest.DEF_MAX_PARAM_NAME;
		testParams.method = testMethod;
		testParams.filterAscending = request.options.hasOwnProperty("optFilterAscending")
			? request.options.optFilterAscending
			: null;
		testParams.filterValue = request.options.hasOwnProperty("optFilterValue")
			? request.options.optFilterValue
			: 50;
		testParams.filterParamName = request.options.hasOwnProperty("optFilterParamName")
			? request.options.optFilterParamName
			: "Total trades: All";
		testParams.deepStartDate =
			!request.options.hasOwnProperty("deepStartDate") ||
			request.options["deepStartDate"] === ""
				? null
				: request.options["deepStartDate"];
		testParams.backtestDelay =
			!request.options.hasOwnProperty("backtestDelay") || !request.options["backtestDelay"]
				? 0
				: request.options["backtestDelay"];
		testParams.randomDelay = request.options.hasOwnProperty("randomDelay")
			? Boolean(request.options["randomDelay"])
			: true;
		testParams.shouldSkipInitBestResult = request.options.hasOwnProperty(
			"shouldSkipInitBestResult"
		)
			? Boolean(request.options["shouldSkipInitBestResult"])
			: false;
		testParams.shouldSkipWaitingForDownload = request.options.hasOwnProperty(
			"shouldSkipWaitingForDownload"
		)
			? Boolean(request.options["shouldSkipWaitingForDownload"])
			: false;
		testParams.dataLoadingTime =
			request.options.hasOwnProperty("dataLoadingTime") &&
			!isNaN(parseInt(request.options["dataLoadingTime"]))
				? request.options["dataLoadingTime"]
				: 30;
	}
...
action._showStartMsg = (paramSpaceNumber, cycles, addInfo) => {
	...
	extraHeader +=
		paramSpaceNumber / cycles > 10
			? `<br />This is too large for ${cycles} cycles. It is recommended to use up to 3-4 essential parameters, remove the rest from the strategy parameters file.`
			: "";
...
action._saveTestResults = async (testResults, testParams, isFinalTest = true) => {
	...
	const CSVResults = file.convertResultsToCSV(testResults);
	const bestResult = testResults.perfomanceSummary ? model.getBestResult(testResults) : {};
	const initBestValue = testResults.hasOwnProperty("initBestValue")
		? testResults.initBestValue
		: null;
	const propVal = {};
	testResults.paramsNames.forEach((paramName) => {
		if (bestResult.hasOwnProperty(`__${paramName}`))
	...
	if (isFinalTest) await tv.setStrategyParams(testResults.shortName, propVal);
	let text = `All done.\n\n`;
	text +=
		bestResult && bestResult.hasOwnProperty(testParams.optParamName)
			? "The best " +
			  (testResults.isMaximizing ? "(max) " : "(min) ") +
			  testParams.optParamName +
			  ": " +
			  backtest.convertValue(bestResult[testParams.optParamName])
			: "";
	text +=
		initBestValue !== null &&
		bestResult &&
		bestResult.hasOwnProperty(testParams.optParamName) &&
		initBestValue === bestResult[testParams.optParamName]
		...
		`All done.\n\n${
			bestResult && bestResult.hasOwnProperty(testParams.optParamName)
				? "The best " +
				  ...
				  testParams.optParamName +
				  ": " +
				  bestResult[testParams.optParamName]
				: ""
		}`
	);
	if (testParams.shouldSkipWaitingForDownload || !isFinalTest)
		file.saveAs(
			CSVResults,
			`${testResults.ticker}:${testResults.timeFrame}${
				testResults.isDeepTest ? " deep backtesting" : ""
			} ${testResults.shortName} - ${testResults.cycles}_${
				testResults.isMaximizing ? "max" : "min"
			}_${testResults.optParamName}_${testResults.method}.csv`
		);
	if (isFinalTest) {
