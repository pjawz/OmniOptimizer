...
export async function runAlgorithm(user_inputs, optimization_results, deps) {
    ...
    if (selected_algorithm === "traversal") {
        await iterateRanges(user_inputs, optimization_results, user_inputs.length - 1, deps.change_tv_input, deps.sleep, deps.optimizeParams);
        ...
        await iterateGenetic(population_size, user_inputs, max_iterations, mutation_probability, optimization_results, (params, inputs, results) => evaluateOutcome(params, inputs, results, deps));
    } else if (selected_algorithm === "bayesian") {
        const max_iterations = 50;
        await iterateBayesian(user_inputs, max_iterations, optimization_results, (params, inputs, results) => evaluateOutcome(params, inputs, results, deps));
    }
}
