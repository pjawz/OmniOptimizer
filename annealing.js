...
async function optAnnealingIteration(allRangeParams, testResults, bestValue, optimizationState) {
	...
	const sign =
		optimizationState.hasOwnProperty("sign") && typeof optimizationState.sign === "number"
			? optimizationState.sign
			: 1;
	...
	const propVal = optAnnealingNewState(
		allRangeParams,
	...
	if (res.data.hasOwnProperty(MAX_PARAM_NAME)) {
		...
		if (
			sign > 0
				? currentEnergy < optimizationState.lastEnergy
				: currentEnergy > optimizationState.lastEnergy
		) {
		...
		if (
			sign > 0
				? optimizationState.lastEnergy < optimizationState.bestEnergy
				: optimizationState.lastEnergy > optimizationState.bestEnergy
		) {
		...
		optimizationState.currentTemp = optAnnealingGetExpTemp(
			initTemp,
			iteration,
			Object.keys(allRangeParams).length
		);
...
function optAnnealingNewState(allRangeParams, temperature, curState) {
	const res = {};
	if (!curState) {
		// || randomInteger(0,1)) {  // for more variable search
...
async function testStrategy(testResults, strategyData, allRangeParams, method = "random") {
	...
	for (let i = 0; i < testResults.cycles; i++) {
		...
		if (!optRes.hasOwnProperty("data")) continue;
		...
		if (
			optimizationState.sign >= 0
				? bestValue === 0
				: bestValue === Object.keys(allRangeParams).length * 65
		) {
			// TODO 2 del
			bestFoundIter = i;
			break;
...
(async () => {
	...
	for (let i = 0; i < 10; i++) {
		...
		let sumRes = 0;
		for (let j = 0; j < 10; j++) {
			const testResults = await testStrategy(
				{ cycles: Math.pow(4, i) + 200 },
				{ name: "test" },
				allRangeParams,
				`annealing`
			);
