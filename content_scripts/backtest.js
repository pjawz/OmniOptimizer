...
backtest.delay = async (backtestDelay = 0, isRandom = true) => {
	...
	if (backtestDelay) {
		...
		if (isRandom) {
			...
			delay = randomInteger(delay10percent, (delay - delay10percent) * 2); // from 0.1 value to 2x value - in average ~ delay == value
		}
...
backtest.testStrategy = async (testResults, strategyData, allRangeParams) => {
	...
	console.log(
		"testStrategy",
		testResults.shortName,
		testResults.isMaximizing ? "max" : "min",
		"value of",
		testResults.optParamName,
		"by",
		testResults.method,
		testResults.filterAscending === null
			? "filter off"
			: "filter ascending" +
					testResults.filterAscending +
					" value " +
					testResults.filterValue +
					" by " +
					testResults.filterParamName,
		testResults.cycles,
		"times"
	);
	...
	if (
		initRes &&
		initRes.hasOwnProperty("bestValue") &&
		initRes.bestValue !== null &&
		initRes.hasOwnProperty("bestPropVal") &&
		initRes.hasOwnProperty("data")
	) {
		...
		try {
			ui.statusMessage(
				`<p>From default and previous test. Best "${
					testResults.optParamName
				}": ${backtest.convertValue(testResults.bestValue)}</p>`
			);
	...
	for (let i = 0; i < testResults.cycles; i++) {
		...
		switch (testResults.method) {
			case "bayesian":
				...
				optRes = await optimizer.optimize(testResults, allRangeParams, optimizationState);
				if (optRes === null) isEnd = true;
				break;
			case "annealing":
				console.log("Annealing optimization");
				optRes = await optAnnealingIteration(
					allRangeParams,
					testResults,
					testResults.bestValue,
					testResults.bestPropVal,
					optimizationState
				);
				break;
			case "sequential":
				console.log("Sequential optimization");
				optRes = await optSequentialIteration(
					allRangeParams,
					testResults,
					testResults.bestValue,
					testResults.bestPropVal,
			...
			case "random":
				...
				optRes = await optAllRandomIteration(
					allRangeParams,
					testResults,
					testResults.bestValue,
					testResults.bestPropVal,
				...
				break;
			case "brute force":
				console.log("Brute force optimization");
				optRes = await optBruteForce(
					allRangeParams,
					testResults,
					testResults.bestValue,
					testResults.bestPropVal,
			...
			default:
				console.log("Random improvement optimization");
				optRes = await optRandomIteration(
					allRangeParams,
					testResults,
					testResults.bestValue,
					testResults.bestPropVal,
		...
		if (
			optRes.hasOwnProperty("data") &&
			optRes.hasOwnProperty("bestValue") &&
			optRes.bestValue !== null &&
			optRes.hasOwnProperty("bestPropVal")
		) {
			testResults.bestValue = optRes.bestValue;
			testResults.bestPropVal = optRes.bestPropVal;
			try {
				let text = `<p>Cycle: ${i + 1}/${
					testResults.cycles
				} (${durationTime}[${setTime}/${parseTime}]/${avgTime} sec). Best "${
					testResults.optParamName
				}": ${backtest.convertValue(testResults.bestValue)}</p>`;
				text += optRes.hasOwnProperty("currentValue")
					? `<p>Current "${testResults.optParamName}": ${backtest.convertValue(
							optRes.currentValue
					  )}</p>`
					: "";
				text +=
					optRes.error !== null
						? `<p style="color: red">${optRes.message}</p>`
						: optRes.message
						? `<p>${optRes.message}</p>`
						: "";
			...
			try {
				let text = `<p>Cycle: ${i + 1}/${testResults.cycles}. Best "${
					testResults.optParamName
				}": ${backtest.convertValue(testResults.bestValue)}</p>`;
				text += optRes.currentValue
					? `<p>Current "${testResults.optParamName}": ${backtest.convertValue(
							optRes.currentValue
					  )}</p>`
					: `<p>Current "${testResults.optParamName}": error</p>`;
				text +=
					optRes.error !== null
						? `<p style="color: red">${optRes.message}</p>`
						: optRes.message
...
async function getInitBestValues(testResults) {
	...
	resData = res["data"];
	if (res["error"] === null) resData = calculateAdditionValuesToReport(resData);

	if (resData && resData.hasOwnProperty(testResults.optParamName)) {
		console.log(
			`Init from current "${testResults.optParamName}":`,
			resData[testResults.optParamName]
		);
		// resVal = resData[testResults.optParamName]
		resData["comment"] = resData["comment"]
			? `Current parameters. ${resData["comment"]}`
			: "Current parameters.";
	...
	if (testResults.startParams.hasOwnProperty("default") && testResults.startParams.default) {
		...
		if (
			resPropVal === null ||
			Object.keys(resPropVal).some((key) => resPropVal[key] !== defPropVal[key])
		) {
			await backtest.delay(testResults.backtestDelay, testResults.randomDelay);
			...
			if (res && res.data && res.data.hasOwnProperty(testResults.optParamName)) {
				console.log(
					`Init from default "${testResults.optParamName}":`,
					res.data[testResults.optParamName]
				);
				res.data["comment"] = res.data["comment"]
					? `Default parameters. ${res.data["comment"]}`
					: "Default parameters.";
			...
			console.log(
				`Default "${testResults.optParamName}" equal current:`,
				resData[testResults.optParamName]
			);
		}
	}
	...
	if (
		!testResults.shouldSkipInitBestResult &&
		testResults.startParams.hasOwnProperty("best") &&
		testResults.startParams.best
	) {
		const isBestIdenticalCurrent =
			testResults.startParams.current &&
			Object.keys(testResults.startParams.current).some(
				(key) => testResults.startParams.current[key] !== testResults.startParams.best[key]
			);
		const isBestIdenticalDefault =
			testResults.startParams.default &&
			Object.keys(testResults.startParams.default).some(
				(key) => testResults.startParams.default[key] !== testResults.startParams.best[key]
			);
		if (resPropVal === null || (!isBestIdenticalCurrent && !isBestIdenticalDefault)) {
			...
			if (res && res.data && res.data.hasOwnProperty(testResults.optParamName)) {
				console.log(
					`Init from best "${testResults.optParamName}":`,
					res.data[testResults.optParamName]
				);
				res.data["comment"] = res.data["comment"]
					? `Best value parameters. ${res.data["comment"]}`
					: "Best value parameters.";
				Object.keys(bestPropVal).forEach(
					(key) => (res.data[`__${key}`] = bestPropVal[key])
				);
				setBestVal(res.data[testResults.optParamName], bestPropVal, res.data);
			}
		} else {
			console.log(
				`Best "${testResults.optParamName}" equal previous (current or default):`,
				resData[testResults.optParamName]
			);
		}
	}
...
backtest.getTestIterationResult = async (
	testResults,
	propVal,
	isIgnoreError = false,
	isIgnoreSetParam = false
) => {
	try {
		tv.isReportChanged = false; // Global value
		let startTime = new Date();
		if (!isIgnoreSetParam) {
			const isParamsSet = await tv.setStrategyParams(
				testResults.shortName,
				propVal,
				testResults.isDeepTest,
				false
			);
			if (!isParamsSet)
				return {
					error: 1,
					errMessage: "The strategy parameters cannot be set",
					data: null,
				};
		}
		...
		if (res.error === null || isIgnoreError) {
			...
			res["data"]["comment"] =
				res["error"] === 2
					...
					: res["error"] === 1
					? "The tradingview calculation process has not started for the strategy based on these parameter values"
					: res["error"] === 3
...
async function getResWithBestValue(res, testResults, bestValue, bestPropVal, propVale) {
	...
	if (Object.hasOwn(res.data, testResults.optParamName)) {
		if (
			...
			if (
				typeof res.data[testResults.filterParamName] !== "number" ||
				(testResults.filterAscending &&
					res.data[testResults.filterParamName] < testResults.filterValue) ||
				(!testResults.filterAscending &&
					res.data[testResults.filterParamName] > testResults.filterValue)
			) {
				isFiltered = true;
				res.data["comment"] = `Skipped for "${
					testResults.filterParamName
				}": ${backtest.convertValue(res.data[testResults.filterParamName])}.${
					res.data["comment"] ? " " + res.data["comment"] : ""
				}`;
		...
		if (isFiltered) testResults.filteredSummary.push(res.data);
		else testResults.perfomanceSummary.push(res.data);
		...
		if (!isFiltered) {
			if (bestValue === null || typeof bestValue === "undefined") {
				...
				res.bestValue =
					bestValue < res.data[testResults.optParamName]
						? res.data[testResults.optParamName]
						: bestValue;
				res.bestPropVal =
					bestValue < res.data[testResults.optParamName] ? propVale : bestPropVal;
				if (bestValue < res.data[testResults.optParamName]) {
					res.isBestChanged = true;
					console.log(
						`Best value max: ${bestValue} => ${res.bestValue}`,
						res.bestPropVal
					);
				} else {
...
function randomInteger(min = 0, max = 10) {
	...
	// max = Math.floor(max);
	return (
		Math.floor((crypto.getRandomValues(new Uint16Array(1))[0] / 65536) * (max - min + 1)) + min
	);
	// return Math.floor(Math.random() * (max - min + 1)) + min;
}

// Random optimization
async function optAllRandomIteration(
	allRangeParams,
	testResults,
	bestValue,
	bestPropVal,
	...
	const propData = optRandomGetPropertiesValues(
		allRangeParams,
		null,
		testResults.paramConditions
	);
	let propVal = propData.data;
	const changedParam = propData.hasOwnProperty("changedParam") ? propData.changedParam : null;
	if (bestPropVal) propVal = expandPropVal(propVal, bestPropVal);

	const res = await backtest.getTestIterationResult(
		testResults,
		propVal,
		false,
		false,
	...
	if (!res || !res.data || res.error !== null) return res;
	res.data["comment"] = res.data["comment"]
		? res.data["comment"] + propData.message
		: propData.message;
	if (!res.message) res.message = propData.message;
	else res.message += propData.message;
	return await getResWithBestValue(res, testResults, bestValue, bestPropVal, propVal);
}

async function optRandomIteration(
	allRangeParams,
	testResults,
	bestValue,
	bestPropVal,
	...
	if (curPropVal) {
		...
		propVal[paramName] =
			diffParams.length === 0
				...
				: diffParams.length === 1
				? diffParams[0]
				: diffParams[randomInteger(0, diffParams.length - 1)];
		msg = `Changed "${paramName}": ${curVal} => ${propVal[paramName]}.`;
	} else {
		allParamNames.forEach((paramName) => {
			propVal[paramName] =
				allRangeParams[paramName][randomInteger(0, allRangeParams[paramName].length - 1)];
		});
...
function expandPropVal(propVal, basePropVal) {
	...
	Object.keys(basePropVal).forEach((key) => {
		if (propVal.hasOwnProperty(key)) newPropVal[key] = propVal[key];
		else newPropVal[key] = basePropVal[key];
	});
	return newPropVal;
}

// Annealing optimization
async function optAnnealingIteration(
	allRangeParams,
	testResults,
	bestValue,
	bestPropVal,
	...
	const isMaximizing = testResults.hasOwnProperty("isMaximizing")
		? testResults.isMaximizing
		: true;
	...
	let propData = optAnnealingNewState(
		allRangeParams,
		optimizationState.currentTemp,
		optimizationState.lastState
	);
	let propVal = propData.data;
	if (bestPropVal) propVal = expandPropVal(propVal, bestPropVal);
	...
	if (!res.data.hasOwnProperty(testResults.optParamName)) return res;
	...
	if (res.hasOwnProperty("isBestChanged") && res.isBestChanged) {
		...
		const expVal = Math.exp(
			-(currentEnergy - optimizationState.lastEnergy) / optimizationState.currentTemp
		); // Math.exp(-10) ~0,000045,  Math.exp(-1) 0.3678 Math.exp(0); => 1
	...
	optimizationState.currentTemp = optAnnealingGetTemp(
		optimizationState.currentTemp,
		testResults.cycles
	);
...
function optAnnealingNewState(allRangeParams, temperature, curState) {
	...
	if (!isAll && curState) {
		...
		if (diffParams.length === 0) {
			...
			const baseOffset = Math.floor(
				temperature * randomNormalDistribution(0, allRangeParams[paramName].length - 1)
			);
			const offsetIndex = (curIndex + sign * baseOffset) % allRangeParams[paramName].length;
			const newIndex2 =
				offsetIndex >= 0 ? offsetIndex : allRangeParams[paramName].length + offsetIndex;
...
async function optAnnealingGetEnergy(testResults, propVal) {
	// TODO 2del test function annealing
	const allDimensionVal = Object.keys(propVal).map((name) =>
		Math.abs(propVal[name] * propVal[name] - 16)
	);
...
async function optBruteForce(
	...
	if (!res.message) res.message = msg;
	else res.message += msg;
	return await getResWithBestValue(res, testResults, bestValue, bestPropVal, propVal);
}

async function optSequentialIteration(
	allRangeParams,
	testResults,
	bestValue,
	bestPropVal,
