...
async function Process() {    
    ...
    if (userTimeFrames == null || userTimeFrames.length <= 0) {
        ...
        for (let i = 0; i < userTimeFrames.length; i++) {
            ...
            let timeIntervalDropdown = document.querySelector("#header-toolbar-intervals div[class*='menuContent']")
            ...
            if (timeIntervalDropdown) {
                timeIntervalDropdown.click();
            } else {
                console.error("timeIntervalDropdown not found");
            }

            let timeIntervalQuery = `div[data-value='${userTimeFrames[i][0]}']`
            await sleep(1000)
            let timeIntervalElem = document.querySelector(timeIntervalQuery);
            if (timeIntervalElem) {
                timeIntervalElem.click();
            } else {
                console.error(`Element for time interval query "${timeIntervalQuery}" not found`);
            }
            await sleep(1000)
...
// Updated ReportBuilder with null checks
function ReportBuilder(reportData, mutation) {
    ...
    if (!reportDataSelector || reportDataSelector.length <= 0) {
        return new Error("report data is not available")
    }
    
    const valueSelector = "[class*='value' i]"
    const currencySelector = "[class*='currency' i]"
    const changeSelector = "[class*='change' i]"

    // Net Profit
    const netProfitValueElem = reportDataSelector[0].querySelector(valueSelector)
    const netProfitCurrencyElem = reportDataSelector[0].querySelector(currencySelector)
    const netProfitChangeElem = reportDataSelector[0].querySelector(changeSelector)
    if (!netProfitValueElem || !netProfitCurrencyElem || !netProfitChangeElem) {
        return new Error("netProfit data is incomplete")
    }
    reportData.netProfit.amount = netProfitValueElem.innerText + ' ' + netProfitCurrencyElem.innerText
    reportData.netProfit.percent = netProfitChangeElem.innerText

    // Max Drawdown
    const maxDrawdownValueElem = reportDataSelector[1].querySelector(valueSelector)
    const maxDrawdownCurrencyElem = reportDataSelector[1].querySelector(currencySelector)
    const maxDrawdownChangeElem = reportDataSelector[1].querySelector(changeSelector)
    if (!maxDrawdownValueElem || !maxDrawdownCurrencyElem || !maxDrawdownChangeElem) {
        return new Error("maxDrawdown data is incomplete")
    }
    reportData.maxDrawdown.amount = maxDrawdownValueElem.innerText + ' ' + maxDrawdownCurrencyElem.innerText
    reportData.maxDrawdown.percent = maxDrawdownChangeElem.innerText

    // Closed Trades
    const closedTradesElem = reportDataSelector[2].querySelector(valueSelector)
    if (closedTradesElem) {
        reportData.closedTrades = closedTradesElem.innerText
    }

    // Percent Profitable
    const percentProfitableElem = reportDataSelector[3].querySelector(valueSelector)
    if (percentProfitableElem) {
        reportData.percentProfitable = percentProfitableElem.innerText
    }

    // Profit Factor
    const profitFactorElem = reportDataSelector[4].querySelector(valueSelector)
    if (profitFactorElem) {
        reportData.profitFactor = profitFactorElem.innerText
    }
}
