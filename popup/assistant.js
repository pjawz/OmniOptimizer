...
function setClickEvents() {
	for (let elId of [
		...
		"testStrategy",
		"deepTestStrategy",
		"previewStrategyTestResults",
		"downloadStrategyTestResults",
		"getStrategyTemplate",
		"uploadStrategyTestParameters",
		"clearAll",
		"show3DChart",
	]) {
		...
		else console.error(`Can not find id ${elId}`);
	}
...
function checkIsTVChart() {
	chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
		...
		if (!tabs[0].url.includes("tradingview.com"))
			message =
				'To work with the extension, activate it on the tab with the opened <a href="https://www.tradingview.com/chart" target="_blank">Tradingview chart</a>.';
		if (
			!tabs[0].url.includes("www.tradingview.com") &&
			!tabs[0].url.includes("en.tradingview.com")
		)
			message =
				'The extension works with the <a href="https://www.tradingview.com/chart" target="_blank">English version</a> of Tradingview.';
		if (message) {
...
function setPopupInputsByOptions(getResults) {
	chrome.storage.local.get("iondvOptions", (getResults) => {
		...
		if (getResults.hasOwnProperty("iondvOptions")) {
			...
			if (
				document.getElementById("optParamName") &&
				iondvOptions.hasOwnProperty("optParamName")
			)
			...
			if (
				iondvOptions.hasOwnProperty("optFilterAscending") &&
				document.getElementById("optFilterMore") &&
				document.getElementById("optFilterLess") &&
				document.getElementById("optFilterOff")
			) {
			...
			if (
				document.getElementById("optFilterValue") &&
				iondvOptions.hasOwnProperty("optFilterValue")
			)
				document.getElementById("optFilterValue").value = iondvOptions.optFilterValue;
			if (
				document.getElementById("optFilterParamName") &&
				iondvOptions.hasOwnProperty("optFilterParamName")
			)
				document.getElementById("optFilterParamName").value =
					iondvOptions.optFilterParamName;

			if (
				document.getElementById("deepStartDate") &&
				iondvOptions.hasOwnProperty("deepStartDate")
			)
				document.getElementById("deepStartDate").value = iondvOptions.deepStartDate;
			if (
				document.getElementById("dataLoadingTime") &&
				iondvOptions.hasOwnProperty("dataLoadingTime")
			) {
				document.getElementById("dataLoadingTime").value =
					iondvOptions.dataLoadingTime !== null ? iondvOptions.dataLoadingTime : 30;
			}
			if (
				document.getElementById("backtestDelay") &&
				iondvOptions.hasOwnProperty("backtestDelay")
			) {
				if (iondvOptions.backtestDelay !== null)
					document.getElementById("backtestDelay").value = iondvOptions.backtestDelay;
			}
			if (
				document.getElementById("randomDelay") &&
				iondvOptions.hasOwnProperty("randomDelay")
			) {
				document.getElementById("randomDelay").checked = Boolean(iondvOptions.randomDelay);
			}
			if (
				document.getElementById("shouldTestTF") &&
				iondvOptions.hasOwnProperty("shouldTestTF")
			) {
				document.getElementById("shouldTestTF").checked = Boolean(
					iondvOptions.shouldTestTF
				);
			}
			if (
				document.getElementById("shouldSkipInitBestResult") &&
				iondvOptions.hasOwnProperty("shouldSkipInitBestResult")
			) {
				document.getElementById("shouldSkipInitBestResult").checked = Boolean(
					iondvOptions.shouldSkipInitBestResult
				);
			}
			if (
				document.getElementById("shouldSkipWaitingForDownload") &&
				iondvOptions.hasOwnProperty("shouldSkipWaitingForDownload")
			) {
				document.getElementById("shouldSkipWaitingForDownload").checked = Boolean(
					iondvOptions.shouldSkipWaitingForDownload
				);
			}
		...
		const link = document.querySelector(
			`div.tabs__links > a${
				typeof tabId === "number" && tabId > 0 && tabId < 3
					? ":nth-child(" + tabId + ")"
					: ""
			}`
		);
		if (link)
			// Activate saved or fist tab
			link.click();
	});
}
...
function getOptions(signal) {
	...
	iondvOptions.optFilterAscending = document.getElementById("optFilterMore").checked
		? true
		: document.getElementById("optFilterLess").checked
		? false
		: null;
	iondvOptions.optFilterValue = document.getElementById("optFilterValue").value || "50";
	iondvOptions.optFilterParamName =
		document.getElementById("optFilterParamName").value || "Total Closed Trades: All";
	const deepStartDateEl = document.getElementById("deepStartDate");
	if (deepStartDateEl) {
		if (
			deepStartDateEl.value === "" ||
			(deepStartDateEl.value && deepStartDateEl.value.match(/^20\d{2}-[0-1][0-9]-[0-3][0-9]/))
		) {
	...
	if (document.getElementById("dataLoadingTime")) {
		...
		iondvOptions.dataLoadingTime =
			dataLoadingTime === ""
				? null
				: isNaN(parseInt(dataLoadingTime))
				? 45
				: Math.abs(parseInt(dataLoadingTime));
	}
	if (document.getElementById("backtestDelay")) {
		const delayValue = document.getElementById("backtestDelay").value;
		iondvOptions.backtestDelay =
			delayValue === ""
				? null
				: isNaN(parseFloat(delayValue))
				? 0.5
				: Math.abs(parseFloat(delayValue));
	}
	...
	iondvOptions.shouldSkipInitBestResult = document.getElementById(
		"shouldSkipInitBestResult"
	).checked;
	iondvOptions.shouldSkipWaitingForDownload = document.getElementById(
...
function sendSignalToActiveTab(signal) {
	...
	chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
		...
		if (iondvOptions) message.options = iondvOptions;
