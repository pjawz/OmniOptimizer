export async function iterateGenetic(population_size, user_inputs, max_iterations, mutation_probability, optimization_results, fitness_function) {
    function initializePopulation(population_size, user_inputs) {
        ...
        for (let i = 0; i < population_size; i++) {
            ...
            user_inputs.forEach((element) => {
                ...
                individual.push(Number((start + Math.random() * (end - start)).toFixed(decimal_places)));
            });
    ...
    async function selection(population, population_size) {
        ...
        for (let i = 0; i < population.length; i++) {
            ...
            await fitness_function(population[i], user_inputs, optimization_results);
        }
    ...
    for (let i = 0; i < max_iterations; i++) {
        ...
        if (!best_individual || current_best[current_best.length - 1] > best_individual[best_individual.length - 1]) {
