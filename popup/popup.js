...
async function ProcessPlusFeatures() {
	...
	await new Promise((resolve) => {
		chrome.runtime.sendMessage(
			{ type: "getAuthToken", isInteractive: false },
			function (response) {
	...
	if (user.is_membership_active) {
		...
		$("#selectTimeFrame").multiselect({
			...
			onChange: async function (option, checked, select) {
				...
				var userTimeFrames = [];
				if (
					Object.keys(userTimeFramesObj).length > 0 &&
					userTimeFramesObj.userTimeFrames != null
				) {
...
function autoFillParameters(parameterNames) {
	...
	for (let i = 0; i < autoFillSelects.length; i++) {
		...
		chrome.storage.local.get(["selectAutoFill" + i], function (result) {
			var userValue = i;
			if (
				result["selectAutoFill" + i] &&
				result["selectAutoFill" + i] <= parameterNames.length - 1
			) {
...
loginButton.addEventListener("click", async () => {
	chrome.runtime.sendMessage(
		{ type: "getAuthToken", isInteractive: true },
		function (response) {}
	);
});
...
function addRemoveParameterBlockEventListener(parameterCount) {
	document
		.querySelectorAll(".btn-close.remove-parameters")
		[parameterCount - 1].addEventListener("click", async (evt) => {
...
function addSaveAutoFillSelectionListener(parameterCount) {
	document
		.querySelectorAll("#selectAutoFill")
		[parameterCount].addEventListener("change", (event) => {
...
async function CreateUserInputsMessage(userInputs) {
	...
	var parameterCount = parameters.children.length;
	var firstAutoFillOptions =
		parameters.children[0].querySelector("#selectAutoFill").options.length;
	...
	for (let i = 0; i < parameterCount; i++) {
		...
		var parameterName =
			parameters.children[i].querySelector("#selectAutoFill").selectedOptions[0].innerText;
...
async function GetMembershipInfo(userEmail) {
	const opGetMembershipInfoURL = "https://api-stg.omnioptimizer.app/api/v1/user/membership/";
